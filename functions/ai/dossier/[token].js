...
function buildPublicKnowledgeMarkdown(posts) {
  if (!posts.length) return "## 公開ナレッジ\n\n- まだ公開ナレッジはありません。";
...
async function readPublicKnowledge(env) {
  const { url } = supabaseConfig(env);
  ...
  return buildPublicKnowledgeMarkdown(posts || []);
}

export async function onRequestGet(context) {
  const token = context.params.token;
  const shareContext = await readShareContext(context.env, token);
  if (!shareContext) {
  ...
  let publicKnowledge = "";
  try {
    publicKnowledge = await readPublicKnowledge(context.env);
  } catch (error) {
