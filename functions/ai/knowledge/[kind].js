...
function buildPublicKnowledgeMarkdown(posts) {
  if (!posts.length) return "# セプターナレッジバンク\n\n- まだ公開ナレッジはありません。";
...
export async function onRequestGet(context) {
  if (context.params.kind !== "public.md" && context.params.kind !== "public") {
    return markdownResponse("# Not found\n\n指定されたナレッジは見つかりません。", 404);
  }

  const { url } = supabaseConfig(context.env);
  ...
  return markdownResponse(buildPublicKnowledgeMarkdown(posts || []));
}
